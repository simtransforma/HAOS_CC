...
description: "Skill frontend anti-slop para landing pages, portfólios e redesigns. O agente lê o briefing, infere a direção de design certa e entrega interfaces que não parecem templated. Use quando o usuário pedir uma landing page, portfólio, redesign de site, hero section, ou disser 'deixa com cara premium', 'estilo Linear/Awwwards', 'não quero cara de template', 'design bonito', 'frontend caprichado'. Sistemas de design reais quando aplicável, auditoria-primeiro em redesigns, checagem de pré-voo rígida. (Corpo técnico em inglês por ser engenharia language-agnostic — regras, hex, código.)"
metadata:
  autor: Gian Marco Menegussi Scaglianti
  marca: HAOS / HAU Soluções Digitais
---

# tasteskill: Skill Frontend Anti-Slop (HAOS)

> Landing pages, portfólios e redesigns. Não dashboards, não tabelas de dados, não UI de produto multi-etapa.
> Toda regra abaixo é **contextual**. Nada dispara automaticamente. Primeiro leia o briefing, depois puxe só o que encaixa.
>
> **Contexto HAOS.** Adapte a direção de design à marca em jogo: **HAU** (técnico/B2B), **EdsonBurger** (provocador/energético), **SIM** (acolhedor, feminino, 55+ — alto contraste, legibilidade, toques grandes), **Editora Mindset** (editorial/educacional). Toda landing pública leva o GTM `GTM-K5DPXJTV` (head + body). Combine com as skills HAOS `mobile-responsiveness`, `vibe-designer` e `design-principles` quando fizer sentido.
>
> **Nota de tradução:** o corpo técnico desta skill (skeletons de código GSAP/Motion, regras de CSS/Tailwind, listas de hex banidos, nomes de pacotes) é mantido em inglês de propósito — são regras de engenharia language-agnostic onde traduzir adicionaria risco sem valor. A identidade, a invocação (acima) e o contexto de marca foram localizados para PT-BR/HAOS.
